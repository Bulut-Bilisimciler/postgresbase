...
> Important Note: Go modules does not support module aliasing. So we changed the module name to "github.com/pocketbase/pocketbase" to "github.com/Bulut-Bilisimciler/postgresbase" in go.mod file and all imported package prefixes. You could change it back to "github.com/pocketbase/pocketbase" if you want to customize the project.  
...
- We use [Twitter Snowflake for ID generation](https://github.com/Bulut-Bilisimciler/postgresbase/blob/master/migrations/1640988000_init.go#L48). Every table ID is generated by postgres function as data type varchar(32)  
- We converted [created and updated columns](https://github.com/Bulut-Bilisimciler/postgresbase/blob/master/migrations/1640988000_init.go#L73-L74) to postgres native date types `TIMESTAMPTZ` to support native date operations  
- We write [json functions for postgres](https://github.com/Bulut-Bilisimciler/postgresbase/blob/master/migrations/1640988000_init.go) in migration files to support json equivalent operations from Pocketbase.  
- We add support [RSA256 JWT Public Private Keys](https://github.com/Bulut-Bilisimciler/postgresbase/blob/master/tools/security/jwt.go) while encoding and decoding token. In our case we need to implement Pocketbase to our existing project with RSA keypair. Currently (Pocketbase v0.20.5) supports symmetric encoding only and we extend it.  
...
git clone https://github.com/Bulut-Bilisimciler/postgresbase
cd postgresbase
go mod download
...
- Install library
```bash
go get github.com/Bulut-Bilisimciler/postgresbase # go version v1.21 or higher
```
...
import (
	...
	pocketbase "github.com/Bulut-Bilisimciler/postgresbase" // ! Just change the import path
	"github.com/Bulut-Bilisimciler/postgresbase/core" // ! Just change the import path
)
